...
export default function MotionProductCard({ product }) {
    ...
    <div className={`bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 group relative flex flex-col h-full ${isOutOfStock ? 'opacity-75' : ''}`}>
      ...
      <Link to={`/products/${product.id}`} className="block shrink-0">
        <div className="aspect-square bg-gray-100 overflow-hidden relative">
          ...
          {/* Gradient Overlay */}
          <div className="absolute inset-0 bg-linear-to-br from-transparent via-transparent to-black/10 pointer-events-none"></div>
          {/* Hover Overlay */}
        ...
        <button 
          ...
          className={`mt-4 w-full flex items-center justify-center gap-2 text-sm font-medium py-2.5 px-4 rounded-xl transition-all duration-200 ${
            isOutOfStock 
              ...
              : 'bg-linear-to-br from-gray-900 to-gray-700 text-white hover:from-gray-800 hover:to-gray-600 active:scale-95 shadow-sm hover:shadow-md'
          }`}
        >
