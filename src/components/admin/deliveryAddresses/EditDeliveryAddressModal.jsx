...
export default function EditDeliveryAddressModal({ isOpen, deliveryAddress, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen && deliveryAddress) {
      setFormData({
        ...
        status: deliveryAddress.status || 'Active',
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await deliveryAddressesService.update(deliveryAddress.id, submitData)
      onSuccess()
      onClose()
    } catch (error) {
      console.error('Failed to update delivery address:', error)
      const errorMessage = error.response?.data?.error || error.response?.data?.message || 'Failed to update delivery address'
  ...
  if (!isOpen || !deliveryAddress) return null
      ...
      <div className="relative min-h-screen flex items-center justify-center p-4">
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Edit Delivery Address</h2>
              <p className="text-sm text-gray-600 mt-0.5">Update delivery address information</p>
            </div>
            <button
              onClick={onClose}
